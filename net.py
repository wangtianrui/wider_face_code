...
def alex_net(image):  # image:[227,227,3]
    with tf.name_scope('alex_net'):
        with tf.name_scope('floor_1'):
            ...
            pool1 = function.pool(conv1, kernel=[3, 3], stride=[1, 2, 2, 1], name='pool1', padding='SAME')
        ...
        with tf.name_scope('floor_2'):
            ...
            pool2 = function.pool(norm2, kernel=[3, 3], stride=[1, 2, 2, 1], name='pool2', padding='VALID')
        with tf.name_scope('fc'):
            fc_1 = function.FC_layer(pool2, 128, "fc1")
            fc_2 = function.FC_layer(fc_1, 128, 'fc2')
            softmax_linear = function.FC_layer(fc_2, 2, 'soft_max')
